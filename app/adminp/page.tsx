...
export default function AdminPyq() {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      setEmail(em);
      if (!em) { setAllowed(false); return; }
      const res = await fetch("/api/admin/pyq", {
        method: "POST",
        headers: await authHeaders(),
        body: JSON.stringify({ action: "ping" }),
      });
      setAllowed(res.status !== 403);
    };
    check();
  }, []);
  ...
  const onFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!f) return;
    if (f.size > 3_500_000) { alert("File too big (max ~3.5 MB). Split long PDFs into pages/images."); return; }
    setFileName(f.name);
    ...
    return (
      <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center max-w-sm w-full">
          ...
          <p className="text-xs text-slate-400">{email ? `${email} is not in ADMIN_EMAILS.` : "Login with an admin account."}</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white px-4 pt-6 pb-24 max-w-3xl mx-auto">
      <div className="mb-5">
        ...
        <p className="text-[11px] text-slate-500 font-semibold mt-1">Upload official paper PDF/photo → Gemini extracts → review → save into bank</p>
      </div>
        ...
        <div className="grid gap-3 mb-4">
          {items.map((q, i) => (
            <div key={i} className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
              ...
              {q.correct_index === -1 && <p className="text-[10px] font-bold text-amber-400 mt-2">⚠️ No answer marked in paper — set correct option above (tap A/B/C/D).</p>}
            </div>
          ))}
