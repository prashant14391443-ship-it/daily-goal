...
import { ArrowLeft, Brain, Sparkles, Star, Copy, Download, Save, Layers, HelpCircle, RefreshCw } from "lucide-react";
...
function summarize(text: string, count: number) {
  ...
  const scored = sents.map((s, i) => {
    ...
    if (i < 2) sc *= 1.2; if (/\d/.test(s)) sc *= 1.15; if (ws.length < 4) sc *= 0.5;
    return { s, i, sc };
  });
  const maxS = Math.max(1, ...scored.map((x) => x.sc));
  const top = [...scored].sort((a, b) => b.sc - a.sc).slice(0, count).sort((a, b) => a.i - b.i);
  const points: Point[] = top.map((x) => { const n = x.sc / maxS; return { text: x.s, importance: Math.max(1, Math.round(n * 5)), highlight: n > 0.7 ? "red" : n > 0.4 ? "orange" : "green" }; });
...
function makeQuiz(points: Point[], text: string): QuizQ[] {
  const freq: Record<string, number> = {}; wordsOf(text).forEach((w) => (freq[w] = (freq[w] || 0) + 1));
...
export default function AiSummary() {
  const [tab, setTab] = useState<"text" | "photo" | "topic">("text");
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const pre = localStorage.getItem("dg-brain-input");
      if (pre) { setInput(pre); localStorage.removeItem("dg-brain-input"); }
    };
    load();
  }, []);
  ...
  const run = async () => {
    if (tab === "photo" && !photo) return flash("⚠️ Choose a photo first!");
    if (tab !== "photo" && !input.trim()) return flash("⚠️ Paste text or type a topic!");
    setBusy(true);
    const count = mode === "quick" ? 5 : mode === "brief" ? 10 : 15;
    try {
      const res = await fetch("/api/ai-summary", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ tab, input, image: photo, count }) });
      if (!res.ok) throw 0;
      const d = await res.json();
      if (d.points?.length) {
        setResult({ title: tab === "topic" ? input : tab === "photo" ? "📷 Photo summary" : input.slice(0, 40) + "…", mode, points: d.points, facts: d.facts || [], overall: d.overall || d.points[0]?.text || "", t: Date.now() });
        setQuiz([]); setQuizIdx(0); setQuizScore(0); setPicked(null); setBusy(false); return;
      }
      throw 0;
    } catch {
      if (tab === "text") {
        const { points, facts, overall } = summarize(input, count);
        setResult({ title: input.slice(0, 40) + "…", mode, points, facts, overall, t: Date.now() });
        setQuiz([]); setQuizIdx(0); setQuizScore(0); setPicked(null);
      } else flash("🌐 AI offline — check GEMINI/GROQ keys, or use 📄 Text.");
    }
    setBusy(false);
  };

  const saveHistory = () => { if (!result) return; const h = [result, ...history].slice(0, 10); setHistory(h); localStorage.setItem("dg-ai-history", JSON.stringify(h)); flash("💾 Saved!"); };
  const copy = () => { if (!result) return; navigator.clipboard.writeText(result.points.map((p, i) => `${i + 1}. ${p.text}`).join("\n")); flash("📋 Copied!"); };
  const download = () => { if (!result) return; const blob = new Blob([result.points.map((p, i) => `${i + 1}. ${p.text}`).join("\n")], { type: "text/plain" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "summary.txt"; a.click(); flash("⬇️ Downloaded!"); };
  const toReview = () => { if (!result) return; result.points.filter((p) => p.importance >= 4).forEach((p) => addTopic(uid, p.text.slice(0, 80))); flash("🔄 Added to Review!"); };
  const toFlashcards = () => { if (!result) return; const cards = result.points.map((p) => ({ front: p.text.split(" ").slice(0, 4).join(" ") + "…?", back: p.text })); const old = JSON.parse(localStorage.getItem("dg-ai-flashcards") || "[]"); localStorage.setItem("dg-ai-flashcards", JSON.stringify([...cards, ...old])); flash("🃏 Flashcards created!"); };
  const toQuiz = () => { if (!result) return; setQuiz(makeQuiz(result.points, result.points.map((p) => p.text).join(" "))); setQuizIdx(0); setQuizScore(0); setPicked(null); };
  const answer = (opt: string) => { if (picked) return; setPicked(opt); if (opt === quiz[quizIdx].answer) setQuizScore((s) => s + 1); };
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white p-4 pb-24 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div><h1 className="text-xl font-bold">AI Study Brain</h1><p className="text-xs text-slate-400">Text / Photo / Topic → points, facts, cards, quiz</p></div>
      </div>

      <div className="flex gap-2 mb-4">
        <button onClick={() => setTab("text")} className={`flex-1 py-2 rounded-xl text-xs font-black border ${tab === "text" ? "bg-cyan-500/20 border-cyan-500/40 text-cyan-300" : "bg-slate-900 border-slate-700 text-slate-400"}`}>📄 Text</button>
        <button onClick={() => setTab("photo")} className={`flex-1 py-2 rounded-xl text-xs font-black border ${tab === "photo" ? "bg-cyan-500/20 border-cyan-500/40 text-cyan-300" : "bg-slate-900 border-slate-700 text-slate-400"}`}>📷 Photo</button>
        <button onClick={() => setTab("topic")} className={`flex-1 py-2 rounded-xl text-xs font-black border ${tab === "topic" ? "bg-cyan-500/20 border-cyan-500/40 text-cyan-300" : "bg-slate-900 border-slate-700 text-slate-400"}`}>🎯 Topic</button>
      </div>

      {tab === "text" ? (
        <textarea value={input} onChange={(e) => setInput(e.target.value)} rows={6} placeholder="Paste any text — chapter, article, notes…" className="w-full p-3 rounded-xl bg-slate-800 border border-slate-700 text-sm outline-none focus:border-cyan-500 mb-3 resize-none" />
      ) : tab === "photo" ? (
        <div className="mb-3">
          <input type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = () => setPhoto(r.result as string); r.readAsDataURL(f); }} className="w-full text-xs text-slate-400 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-cyan-600 file:text-white file:text-xs file:font-black mb-2" />
          {photo && <img src={photo} alt="preview" className="rounded-xl border border-slate-700 max-h-40 object-cover" />}
        </div>
      ) : (
      ...
      <button onClick={run} disabled={busy} className="w-full py-3 rounded-xl bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 font-black flex items-center justify-center gap-2 mb-4"><Sparkles size={16} /> {busy ? "Thinking…" : "Summarize"}</button>
      {msg && <p className="text-center text-xs font-bold text-cyan-300 mb-3">{msg}</p>}

      {result && (
        <>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-4 mb-4"><p className="text-[10px] font-black text-cyan-400 mb-1">💡 OVERVIEW</p><p className="text-sm text-slate-300">{result.overall}</p></div>
          ...
          {result.facts.length > 0 && (<><p className="text-xs font-black text-slate-400 mb-2">🔑 KEY FACTS</p><div className="flex flex-wrap gap-1.5 mb-4">{result.facts.map((f, i) => <span key={i} className="px-2 py-1 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-300 text-[10px]">{f.slice(0, 60)}</span>)}</div></>)}
          ...
          {quiz.length > 0 && quizIdx >= quiz.length && (
            <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 text-center mb-4"><p className="text-3xl mb-2">{quizScore >= quiz.length * 0.7 ? "🏆" : "📚"}</p><p className="text-xl font-black">{quizScore}/{quiz.length}</p></div>
          )}
        </>
      )}

      {history.length > 0 && (<><p className="text-xs font-black text-slate-400 mb-2">🕘 HISTORY</p><div className="grid gap-2">{history.map((h, i) => (<button key={i} onClick={() => setResult(h)} className="bg-slate-900 border border-slate-700 rounded-xl p-3 text-left text-sm text-slate-300 hover:border-slate-600">{h.title}</button>))}</div></>)}
    </main>
  );
}