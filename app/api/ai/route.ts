...
1. Max 2-3 short sentences. Plain text only. No markdown, no bullets, no emojis, no lists.
2. Never read data like a report. Weave it in naturally ("you already studied 40 minutes, that's solid!").
3. Always end with ONE simple question, like a curious friend would.
4. If the user interrupts or changes topic, flow with it instantly. Never mention the interruption.
...
TALK LIKE A REAL HUMAN:
1. Max 2-3 short sentences. Plain text only. No markdown, no emojis.
2. Be warm, spontaneous, and conversational. 
3. Always end with ONE simple, open-ended question to keep the chat going.
4. If they change the subject, flow with it instantly.`;
    } else {
       system = `You are a helpful AI assistant. Keep replies under 100 words.`;
    }
    const chatMessages = [
