    ...
    <div className="fixed inset-0 z-[90] bg-black/70 flex items-end md:items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5 w-full max-w-sm grid gap-3">
          ...
          {CATS.map((c) => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-full border ${
                cat === c ? "bg-violet-600 border-violet-500 text-white" : "bg-slate-800 border-slate-700 text-slate-300"
              }`}
            >
              {c}
            </button>
        ...
        <div className="flex gap-2">
          <button onClick={submit} disabled={saving} className="flex-1 py-2.5 rounded-xl bg-violet-600 font-bold text-white disabled:opacity-50">
            {saving ? "..." : "🚀 Send"}
          </button>
          <button onClick={onClose} className="px-4 py-2.5 rounded-xl bg-slate-800 text-slate-300 font-bold">✖</button>
        </div>
        {sent && <p className="text-green-400 text-sm text-center font-bold">✅ Thank you! You're shaping the app!</p>}
      </div>
    </div>
  );
}