...
import { Code2, BookOpen, Dumbbell, ListChecks, ListTodo, RefreshCw, ArrowRight, Check, type LucideIcon } from "lucide-react";
...
export default function TodayLoop() {
  ...
  const load = async () => {
    ...
    const [study, goals, gym, habits, habitLogs, todo, general, en, pr] = await Promise.all([
      ...
      supabase.from("learning_enrollments").select("track_id").eq("user_id", uid).eq("status", "active").maybeSingle(),
      supabase.from("learning_progress").select("track_id,milestone_id,status").eq("user_id", uid),
    ]);

    // FIX 1: Extract `.data` from the Supabase single-row response
    const g = goals.data || { study_target: 120, workout_target: 1, habits_target: 3 };
    
    const sMin = (study.data || []).reduce((s, r) => s + r.duration_minutes, 0);
    const gymN = (gym.data || []).length;
    const hTot = (habits.data || []).length;
    const hDone = new Set((habitLogs.data || []).map((l) => l.habit_id)).size;
    const tRows = [...(todo.data || []), ...(general.data || [])];
    const tDone = tRows.filter((t) => t.completed).length;
    
    // Note: If countDue accesses a database asynchronously, you may need to add `await` here
    const due = countDue(uid); 

    const list: StepItem[] = [];

    // FIX 2: Check `en.data?.track_id` instead of `en?.track_id`
    if (en.data?.track_id) {
      const tr = getTrackById(en.data.track_id);
      if (tr && tr.milestones.length > 0) {
        const doneIds = new Set(
          (pr.data || [])
            .filter((p: any) => p.track_id === tr.id && p.status === "completed")
  ...
  return (
    <div className="mb-4 rounded-2xl border border-emerald-400/25 bg-gradient-to-br from-emerald-500/10 to-teal-500/5 p-4">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[10px] font-black uppercase tracking-widest text-emerald-300">Today&apos;s loop</p>
        <p className="text-[10px] font-bold text-slate-400">{doneN}/{steps.length} done</p>
      </div>
      ...
      {next ? (
        <button onClick={() => router.push(next.href)}
          ...
          <span className="w-9 h-9 shrink-0 rounded-lg bg-emerald-500/20 flex items-center justify-center">
            <next.icon size={16} className="text-emerald-300" />
          </span>
        ...
        <p className="mb-3 text-center text-[12px] font-black text-emerald-300">🎉 All done today — rest or review!</p>
      )}
