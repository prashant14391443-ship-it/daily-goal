...
function Inner() {
  ...
  const saveProfile = async () => {
    ...
    const { error: saveErr } = await supabase.from("profiles").upsert(
      { user_id: me, bio: editBio, display_name: finalName, avatar_url: newAvatarUrl },
      { onConflict: "user_id" }
    );
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white pb-24">
      ...
      <div className="flex items-center justify-between p-4">
        ...
        <p className="font-black text-lg truncate max-w-[50%]">
          {prof?.is_private ? "🔒 " : ""}
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-full bg-amber-400/10 border border-amber-400/30 text-amber-400 text-xs font-black hover:bg-amber-400/20 transition-colors"
        >
          🪙 {coins}
        </button>
      </div>
      ...
      {/* HERO CARD */}
      <div className="mx-4 rounded-2xl bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 p-4 grid gap-4">
        <div className="flex items-center gap-4">
          <span className="rounded-full bg-gradient-to-tr from-amber-400 via-pink-500 to-fuchsia-600 p-[2px] shrink-0">
            {prof?.avatar_url ? (
              <img src={prof.avatar_url} className="w-20 h-20 rounded-full object-cover border-2 border-slate-950" alt="" />
            ) : (
              <span className="w-20 h-20 rounded-full bg-violet-600 border-2 border-slate-950 flex items-center justify-center text-3xl font-bold">
          ...
          <div className="flex-1 grid grid-cols-3 text-center">
            ...
            <button onClick={() => setRankOpen(true)} className="hover:scale-105 transition-transform">
              <p className="font-black text-lg">{lvl.icon}</p>
              <p className="text-[10px] text-slate-400">{lvl.name}</p>
            </button>
          </div>
        </div>

        {/* SEASON PROGRESS */}
        <button onClick={() => setRankOpen(true)} className="text-left grid gap-1.5">
          <div className="flex justify-between text-[10px] text-slate-400">
            <span>🏁 {lvl.icon} {lvl.name}</span>
            <span>
              {lvl.next ? `${lvl.next.need - coins} 🪙 to ${lvl.next.icon} ${lvl.next.name}` : "🎓 CHAMPION"} • ⏳ {daysLeft}d
            </span>
          </div>
          <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${lvl.next ? Math.min(100, ((coins - lvl.need) / (lvl.next.need - lvl.need)) * 100) : 100}%`,
              }}
            />
          </div>
        </button>
        ...
        {userId === me ? (
          <>
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-bold transition-colors"
            >
              {editing ? "✖ Close" : "✏️ Edit profile"}
            </button>
            <button onClick={share} className="flex-1 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-bold transition-colors">
              📤 Share profile
            </button>
            <button onClick={togglePrivate} className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-bold transition-colors">
              {prof?.is_private ? "🔒" : "🌍"}
            </button>
          </>
        ) : (
          <>
            {isFriend ? (
              <span className="flex-1 py-2.5 rounded-xl bg-slate-800 text-xs font-bold text-center text-green-400">✓ Friends</span>
            ) : recvReq ? (
              <button onClick={accept} className="flex-1 py-2.5 rounded-xl bg-green-600 hover:bg-green-500 text-xs font-bold transition-colors">
                ✅ Accept Request
              </button>
            ) : sentReq ? (
              <span className="flex-1 py-2.5 rounded-xl bg-slate-800 text-xs font-bold text-center text-slate-400">⏳ Requested</span>
            ) : (
              <button onClick={addFriend} className="flex-1 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-500 text-xs font-bold transition-colors">
                ➕ Add Friend
              </button>
            )}
            {(!prof?.is_private || isFriend) && (
              <Link href={`/chat?user=${userId}`} className="flex-1 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-bold text-center transition-colors">
                💬 Message
              </Link>
            )}
          </>
      ...
      {editing && userId === me && (
        <div className="px-4 mb-4 grid gap-2">
          <input
            ...
            className="p-3 rounded-xl bg-slate-800 border border-slate-700 text-sm"
          />
          <textarea
            value={editBio}
            ...
            rows={2}
            className="p-3 rounded-xl bg-slate-800 border border-slate-700 text-sm resize-none"
          />
          <label className="py-2 rounded-xl bg-slate-800 text-xs font-bold text-center cursor-pointer">
            📷 Change photo
            ...
            <div className="grid gap-2 bg-slate-800/50 rounded-xl p-3">
              <div
                ...
                onPointerMove={(e) => {
                  ...
                  setOff({ x: off.x + (e.clientX - dragRef.current.x), y: off.y + (e.clientY - dragRef.current.y) });
                  dragRef.current = { x: e.clientX, y: e.clientY };
                }}
          ...
          <button onClick={saveProfile} disabled={saving} className="py-2.5 rounded-xl bg-violet-600 text-xs font-bold disabled:opacity-50">
            💾 Save
          </button>
        </div>
      )}
      ...
      {locked ? (
        ...
        <div className="text-center py-16 px-6">
          ...
          {userId === me && (
            <Link href="/newpost" className="inline-block mt-4 px-8 py-3 rounded-xl bg-violet-600 font-bold text-sm">
              Create
            </Link>
          )}
        </div>
          ...
          {posts.map((p) => (
            <div key={p.id} onClick={() => setView(p)} className="aspect-square bg-slate-900 relative overflow-hidden cursor-pointer">
              {p.image_url ? (
                ...
                <div
                  className={`relative w-full h-full ${p.bgc ? "" : `bg-gradient-to-br ${BGS[(p.bg || 0) % BGS.length]}`}`}
      ...
      {view && (
        <div className="fixed inset-0 z-[70] bg-black/95 flex flex-col" onClick={() => setView(null)}>
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-8" onClick={(e) => e.stopPropagation()}>
            ...
            {!view.image_url && (
              <div
                className={`relative w-full aspect-square rounded-xl ${view.bgc ? "" : `bg-gradient-to-br ${BGS[(view.bg || 0) % BGS.length]}`}`}
            ...
            {view.user_id === me && (
              <button onClick={() => { deletePost(view); setView(null); }} className="mt-3 w-full py-2 rounded-xl bg-red-600/20 text-red-400 text-xs font-bold">
                🗑 Delete post
              </button>
            )}
            <button onClick={() => setView(null)} className="mt-3 w-full py-2 rounded-xl bg-slate-800 text-xs font-bold">
              Close
            </button>
          </div>
        </div>
      ...
      {rankOpen && (
        <div className="fixed inset-0 z-[80] bg-black/80 flex items-end sm:items-center sm:justify-center" onClick={() => setRankOpen(false)}>
          <div className="w-full max-w-md bg-slate-900 rounded-t-2xl sm:rounded-2xl p-5 grid gap-4 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center">
              <p className="font-black">🎖 Ranks</p>
              <button onClick={() => setRankOpen(false)} className="text-slate-400 text-xl">✖</button>
            </div>

            <div className="rounded-xl bg-slate-950 border border-slate-800 p-4 grid gap-2">
              ...
              {lvl.next ? (
                <>
                  <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-amber-400 to-fuchsia-500"
                      style={{ width: `${Math.min(100, ((coins - lvl.need) / (lvl.next.need - lvl.need)) * 100)}%` }}
                    />
                  </div>
                  <p className="text-[10px] text-slate-500">
                    {lvl.next.need - coins} 🪙 to {lvl.next.icon} {lvl.next.name} • ⏳ resets in {daysLeft}d
                  </p>
                </>
              ) : (
                <p className="text-[10px] text-amber-400 font-bold">🎓 SEASON CHAMPION!</p>
              )}
            </div>
            ...
            <div className="rounded-xl bg-slate-950 border border-slate-800 p-4 grid gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-black">
                    {life >= LIFETIME_LEVELS[0].need ? lifeLvl.name : "Rookie"}
                  </p>
              ...
              {life < LIFETIME_LEVELS[0].need && (
                <p className="text-[10px] text-slate-500">
                  {(64000 - life).toLocaleString()} 🪙 to 👑 Legend
                </p>
              )}
            </div>

            {trophies.length > 0 && (
              <p className="text-xs text-amber-400 text-center">🏆 {trophies.join(" • ")}</p>
            )}
          </div>
        </div>
      )}
            ...
            <div className="rounded-xl bg-slate-950 border border-slate-800 p-4 grid gap-2">
              ...
              {EARN.map((e) => (
                <div key={e.name} className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-bold text-amber-400">+{e.coins}</span>
                </div>
              ))}
            </div>

            <div className="rounded-xl bg-slate-950 border border-slate-800 p-4 grid gap-1.5">
              ...
              {SEASON_LEVELS.map((l) => (
                <div
                  ...
                  className={`flex items-center justify-between text-sm rounded-lg px-2 py-1 ${
                    lvl.name === l.name ? "bg-violet-600/20 border border-violet-500/40" : ""
                  }`}
                >
            ...
            <div className="rounded-xl bg-slate-950 border border-slate-800 p-4 grid gap-1.5">
              ...
              {LIFETIME_LEVELS.map((l) => (
                <div
                  key={l.name}
                  className={`flex items-center justify-between text-sm rounded-lg px-2 py-1 ${
                    life >= LIFETIME_LEVELS[0].need && lifeLvl.name === l.name
