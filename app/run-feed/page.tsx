...
function RouteMap({ route, size = 56 }: { route: RoutePt[] | null; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" className="rounded-xl bg-slate-800/60 shrink-0">
      <polyline points={pts} fill="none" stroke="#22c55e" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
    ...
    <main className="min-h-screen bg-slate-950 text-white px-4 pt-6 pb-24 max-w-2xl mx-auto">
      ...
      {posts.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-[11px] text-slate-500 mt-1">Finish a track & tap "Post my run" to appear here.</p>
        </div>
      ) : (
        <div className="grid gap-3">
          {posts.map((f) => (
            <div key={f.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-3 flex items-center gap-3">
              <RouteMap route={f.route} size={56} />
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-bold text-white truncate">{f.display_name} · {f.mode}</p>
                <p className="text-[10px] text-slate-500">{f.distance_km} km · {fmtTime(f.duration_sec || 0)} · {f.calories} kcal</p>
                <p className="text-[9px] text-slate-600 mt-0.5">{new Date(f.created_at).toLocaleDateString()}</p>
              </div>
              <div className="shrink-0 text-right">
                <p className="text-[11px] font-black text-amber-300">+{f.coins} 🪙</p>
                {f.user_id === uid && (
                  <button onClick={() => del(f.id)} className="mt-1 w-8 h-8 rounded-lg bg-rose-500/10 border border-rose-500/30 text-rose-300 flex items-center justify-center"><Trash2 size={13} /></button>
                )}
              </div>
            </div>
          ))}
