...
import { ALL_EXAMS, getExamById } from "@/lib/examPatterns";
...
export default function SeederPage() {
  ...
  const allTopics = exam.sections.flatMap((s) => s.topics.map((t) => ({ ...t, section_id: s.id })));
  ...
  const extract = async () => {
    ...
    try {
      const res = await fetch("/api/seeder", { method: "POST", headers: await authHeaders(), body: JSON.stringify({ action: "extract", dataUrl: file }) });
      const d = await res.json();
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch("/api/seeder", {
        ...
        body: JSON.stringify({ action: "save", exam_id: examId, year, questions: valid }),
      });
      const d = await res.json();
      if (!res.ok) throw new Error(d.error || "Save failed");
      setMsg(`💾 Saved ${d.saved} real ${year} questions into the ${exam.name} bank!`);
      setItems([]); setFile(null); setFileName("");
    } catch (e: any) { setMsg(`❌ ${e.message}`); }
    setSaving(false);
  };

  if (allowed === null) return <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center"><Loader2 className="animate-spin" /></main>;
  if (!allowed) {
    return (
      <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center p-6">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center max-w-sm w-full">
          ...
          <p className="text-xs text-slate-400">Login with an ADMIN_EMAILS account to use the seeder.</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white px-4 pt-6 pb-24 max-w-3xl mx-auto">
      <div className="mb-5">
        ...
        <p className="text-[11px] text-slate-500 font-semibold mt-1">Upload paper PDF/photo → Gemini extracts ALL questions → review → save</p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-4">
        ...
        <div className="flex items-center gap-2 mt-3 flex-wrap">
          <p className="text-[10px] font-black text-slate-400 uppercase">Exam:</p>
          <div className="flex gap-1.5 flex-wrap">
            {ALL_EXAMS.map((e) => (
              <button key={e.id} onClick={() => setExamId(e.id)} className={`press px-2.5 py-1 rounded-lg text-[10px] font-black border ${examId === e.id ? "bg-emerald-500/20 border-emerald-500/50 text-emerald-300" : "bg-slate-800 border-slate-700 text-slate-400"}`}>
                {e.name.split("(")[0].trim()}
              </button>
            ))}
          </div>
        ...
        <div className="grid gap-3 mb-4">
          {items.map((q, i) => (
            <div key={i} className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
              ...
              {q.correct_index === -1 && <p className="text-[10px] font-bold text-amber-400 mt-2">⚠️ No answer marked — tap the correct option letter above.</p>}
            </div>
          ))}
      ...
      {items.length === 0 && !extracting && (
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 text-center">
          ...
          <p className="text-xs text-slate-500 font-semibold">Saved questions automatically mix into that exam's PYQ papers (real ones get priority).</p>
        </div>
      )}
    </main>
  );
