...
import { BookOpen, Timer, Layers, Brain, Bot, RefreshCw, Sparkles } from "lucide-react";
    ...
    <main className="min-h-screen bg-slate-950 text-white px-4 pt-16 pb-24 max-w-4xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center gap-3">
          <span className="w-11 h-11 shrink-0 rounded-xl bg-blue-500/10 text-blue-400 flex items-center justify-center"><BookOpen size={22} strokeWidth={2.2} /></span>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link href="/studylog" className="press bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="w-9 h-9 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center mb-4"><BookOpen size={18} strokeWidth={2.2} /></span>
        ...
        <Link href="/focus" className="press bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="w-9 h-9 rounded-lg bg-red-500/10 text-red-400 flex items-center justify-center mb-4"><Timer size={18} strokeWidth={2.2} /></span>
        ...
        <Link href="/flashcards" className="press bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="w-9 h-9 rounded-lg bg-violet-500/10 text-violet-400 flex items-center justify-center mb-4"><Layers size={18} strokeWidth={2.2} /></span>
        ...
        <Link href="/summarize" className="press bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="w-9 h-9 rounded-lg bg-pink-500/10 text-pink-400 flex items-center justify-center mb-4"><Brain size={18} strokeWidth={2.2} /></span>
          <p className="font-black text-sm text-white">Summarize</p>
          <p className="text-[10px] text-slate-500 mt-0.5">Points + mind-map</p>
        </Link>
        <Link href="/quiz" className="press bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="w-9 h-9 rounded-lg bg-cyan-500/10 text-cyan-400 flex items-center justify-center mb-4"><Bot size={18} strokeWidth={2.2} /></span>
          <p className="font-black text-sm text-white">AI Quiz</p>
          <p className="text-[10px] text-slate-500 mt-0.5">Any topic → instant test</p>
        </Link>
        <Link href="/ai-summary" className="press bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <span className="w-9 h-9 rounded-lg bg-teal-500/10 text-teal-400 flex items-center justify-center mb-4"><Sparkles size={18} strokeWidth={2.2} /></span>
          <p className="font-black text-sm text-white">AI Study Brain</p>
          <p className="text-[10px] text-slate-500 mt-0.5">Summarize → cards + quiz</p>
        </Link>
        <Link href="/review" className="press bg-slate-900 border border-slate-800 rounded-2xl p-4 hover:border-slate-700 transition-colors col-span-2">
          <div className="w-11 h-11 rounded-xl bg-teal-500/10 text-teal-400 flex items-center justify-center mb-6"><RefreshCw size={20} /></div>
