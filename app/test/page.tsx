...
export default function TestHub() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white px-4 pt-6 pb-24 max-w-4xl mx-auto">
      ...
      >
        {starting === "full" ? (
          <><Loader2 size={18} className="animate-spin" /> Generating paper...</>
        ) : (
              ...
              {starting === s.id ? (
                ...
                <>
                  ...
                  <p className="text-[10px] text-slate-500 font-bold mt-0.5">
                    {s.questionCount} Qs • {s.questionCount * s.marksPerQ} marks
                  </p>
                </>
              )}
            </button>
      ...
      <div className="bg-slate-900 border border-violet-500/20 rounded-2xl p-4 mb-4">
        ...
        >
          {starting === `pyq-${pyqYear}` ? (
            <><Loader2 size={16} className="animate-spin" /> Recreating {pyqYear} paper...</>
          ) : (
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-4">
          ...
          {SSC_CGL_T1.sections.map((s, i) => (
            <div key={s.id} className="flex items-center justify-between bg-slate-800/60 rounded-xl p-3">
              <div className="flex items-center gap-3">
                <span className="w-7 h-7 rounded-lg bg-slate-700 flex items-center justify-center text-xs font-black text-white">
                  {i + 1}
                </span>
        ...
        <div className="flex items-start gap-2 mt-3 bg-amber-500/10 border border-amber-500/20 rounded-xl p-3">
          ...
          <p className="text-[11px] text-amber-200 font-semibold">
            +2 per correct, −0.5 per wrong. Timer auto-submits at 0:00.
          </p>
        </div>
      </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-white">
                      {a.status === "completed" ? `Score: ${a.final_score}/${(a.total_questions || 100) * 2}` : "In Progress"}
                      <span className="text-[10px] text-slate-500 font-bold ml-2">{a.total_questions} Qs</span>
                      {a.year && (
                        <span className="text-[10px] font-black text-violet-400 ml-2">PYQ {a.year}</span>
                      )}
                    </p>
                ...
                {a.status === "completed" ? (
                  <span className={`text-sm font-black ${
                    a.accuracy >= 60 ? "text-emerald-400" : 
                    a.accuracy >= 40 ? "text-amber-400" : 
                    "text-red-400"
                  }`}>
