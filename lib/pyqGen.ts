...
export function validParsed(p: any): boolean {
  return !!p && typeof p.question === "string" && Array.isArray(p.options) && p.options.length === 4 && typeof p.correct === "number" && p.correct >= 0 && p.correct <= 3;
}
    ...
    for (const model of GROQ_MODELS) {
      try {
        const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
          ...
          body: JSON.stringify({ model, messages: [{ role: "user", content: prompt }], max_tokens: 1200, temperature: 0.8 }),
        });
        if (r.ok) {
    ...
    for (const model of GEMINI_MODELS) {
      try {
        const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${keys.gemini}`, {
          ...
          body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { temperature: 0.8, maxOutputTokens: 1500, responseMimeType: "application/json" } }),
        });
        if (r.ok) {
          const d = await r.json();
